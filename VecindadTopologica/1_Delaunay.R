...
setwd("C:/Users/CEDEUS 18/Documents/CEDEUS/Monica - 2018/15_TesisRT/Data") #desktop Monica
setwd("C:/Users/rtruf/Desktop/WORK/TESIS_DISC/DATA") #laptop Ricardo
...
comunas <- c(13101, 13102, 13103, 13104, 13105, 13106, 13107, 13108, 13109, 13110, 13111, 13112, 13113, 13114,
             ...
             13131, 13132, 13201, #13203, San Jose de Maipo falla - solo tiene 3 manzanas con POB Valida dentro del AUC
...
for (comuna in comunas) {
  ...
  plot(vecs1, coords) # Plotear triangulacion
  
  # Fijar conjunto de vecinos ------------------------------------------
  
  metros <- 900 # Definir distancia maxima entre vecinos en metros
  
  # Funcion obtiene vecinos que esten a menos de 900 metros o un minimo de 3 vecinos
  thresh <- foreach(v = 1:length(vecs1)) %do%  {
    ...
    # Aplicar funcion calculo de distancia a cada vecino de cada punto
    e <- apply(neighs,1, function(x) {
      d <- sqrt((x[1] - source[1])^2 + (x[2] - source[2])^2) # Distancia en metros
      d
    })
  
    # Ordenar y encontrar vecinos minimos y maximos
    orden <- e[order(e)] # Ordenar por distancia
    minimos <- orden[1:2] # Obtener los 2 vecinos mcs cercanos
    maximos <- orden[1:min(3,length(e))] # Obtener los vecinos mas lejanos
  
   # Vecino en menos de 900 metros o dentro del conjunto de minimos y maximos
    f <- unlist(lapply(1:length(e), function(x) { # asigna False
      (e[x] < metros | # distancia menor a maximo en metros o
        names(e[x]) %in% names(minimos)) & # Vecinos en el conjunto de minimo Y
        names(e[x]) %in% names(maximos) # Vecinos en el cojunto de maximos
    }))
  
  }
  
  # Aplicar funcion thresh a listado de vecinos
  vecs <- vecs1 # Definir objeto vecs
  for (obs in 1:length(vecs1)) {
    vecs[[obs]] <- vecs[[obs]] [thresh[[obs]]]
  }
  
  plot(vecs,coords) # Plotear triangulacion final
  
  # Traspasar vecindad a lineas
